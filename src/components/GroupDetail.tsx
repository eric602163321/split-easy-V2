...
import {
  Plus, Trash2, Utensils, Shirt, Home, Car, PartyPopper, MoreHorizontal,
  Calculator, Check, ChevronDown, ArrowLeft, UserCheck, ArrowRightLeft,
} from "lucide-react";
import { Button } from "@/components/ui/button";
import { SettlementStats } from "@/components/SettlementStats";
import {
  type Member, type Group, type GroupExpense, type Settlement,
...
export function GroupDetail({ group, onBack, onGroupUpdate }: Props) {
  ...
  const resetForm = () => {
    setAmount(""); setNote(""); setCategory("food"); setPayerId(""); setSplitAmong([]); setShowForm(false); setShowPayerDropdown(false);
  };
  ...
  const handleAdd = () => {
    ...
    const updated = addGroupExpense({
      groupId: group.id, amount: Number(amount), category, payerId, splitAmong,
  ...
  const perPerson = splitAmong.length > 0 && Number(amount) > 0
    ? Math.round((Number(amount) / splitAmong.length) * 100) / 100 : 0;

  return (
    <div className="flex flex-col gap-4 pb-4">
      {/* Header */}
      <div className="flex items-center gap-3">
        <button onClick={onBack} className="touch-target w-10 h-10 rounded-full bg-secondary flex items-center justify-center active:scale-95 transition-transform">
          <ArrowLeft className="w-5 h-5 text-foreground" />
        </button>
        <div className="flex-1 min-w-0">
          <h2 className="font-bold text-foreground text-lg truncate">{group.name}</h2>
          <p className="text-xs text-muted-foreground">{groupMembers.length} 位成員 · {baseCurrencySymbol}</p>
        </div>
      </div>
      ...
      <div className="ios-card p-4">
        ...
        <AnimatePresence>
          {showMemberPicker && (
            <motion.div className="mt-3 flex flex-col gap-1 bg-secondary/50 rounded-xl p-2" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}>
              {allMembers.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-3">請先到成員管理新增朋友</p>
              ) : allMembers.map((m) => {
                const inGroup = group.memberIds.includes(m.id);
                return (
                  <button key={m.id} onClick={() => toggleGroupMember(m.id)}
      ...
      {groupMembers.length > 0 && (
        <>
            ...
            {showForm && (
              <motion.div className="ios-card p-4 flex flex-col gap-4"
                    ...
                    {CATEGORIES.map((cat) => {
                      ...
                      return (
                        <button key={cat.key} onClick={() => setCategory(cat.key)}
                          ...
                          <Icon className="w-5 h-5" /><span className="text-xs">{cat.label}</span>
                        </button>
                      );
                    })}
                  </div>
                ...
                <div className="relative">
                  ...
                  <button onClick={() => setShowPayerDropdown(!showPayerDropdown)}
                  ...
                  <AnimatePresence>
                    {showPayerDropdown && (
                      <motion.div className="absolute z-10 top-full mt-1 w-full bg-background rounded-xl shadow-lg border border-border overflow-hidden"
                        initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }}>
                        {groupMembers.map((m) => (
                          <button key={m.id} onClick={() => { setPayerId(m.id); setShowPayerDropdown(false); }}
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button onClick={() => setSplitAmong(splitAmong.length === groupMembers.length ? [] : groupMembers.map((m) => m.id))}
                      className="text-xs font-medium text-ios-blue">
                      {splitAmong.length === groupMembers.length ? "取消全選" : "全選"}
                    </button>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {groupMembers.map((m) => {
                      ...
                      return (
                        <button key={m.id} onClick={() => toggleSplit(m.id)}
                          className={`flex items-center gap-1.5 px-3 py-2 rounded-full text-sm font-medium transition-all active:scale-95 ${selected ? "bg-ios-blue text-primary-foreground shadow" : "bg-secondary text-secondary-foreground"}`}>
                ...
                {perPerson > 0 && (
                  <motion.div className="bg-secondary rounded-xl p-3 text-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    <p className="text-sm text-muted-foreground">
                      每人分擔 <span className="font-bold text-foreground text-lg">{baseCurrencySymbol} {perPerson}</span>
                    </p>
                  </motion.div>
                )}

                <div className="flex gap-3">
                  <Button variant="iosSecondary" size="lg" className="flex-1" onClick={resetForm}>取消</Button>
                  <Button variant="ios" size="lg" className="flex-1" onClick={handleAdd}
                    disabled={!payerId || !amount || Number(amount) <= 0 || splitAmong.length === 0}>確認</Button>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <AnimatePresence>
            {showSettle && (
              <motion.div className="ios-card p-4 flex flex-col gap-3"
                initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }}>
                <div className="flex items-center justify-between">
                  <p className="font-semibold text-foreground text-lg">結算</p>
                  <Button variant="iosGhost" size="sm" onClick={() => setShowSettle(false)}>關閉</Button>
                </div>
                ...
                <div className="bg-secondary/50 rounded-xl p-3 flex flex-col gap-2">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <div className="relative flex-1">
                      <button onClick={() => setShowTargetCurrencyDropdown(!showTargetCurrencyDropdown)}
                      ...
                      <AnimatePresence>
                        {showTargetCurrencyDropdown && (
                          <motion.div className="absolute z-20 top-full mt-1 w-full bg-background rounded-xl shadow-lg border border-border overflow-hidden max-h-36 overflow-y-auto"
                            initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }}>
                            {CURRENCIES.map((c) => (
                              <button key={c.code} onClick={() => { setTargetCurrency(c.code); setShowTargetCurrencyDropdown(false); if (c.code === group.baseCurrency) setExchangeRate("1"); }}
                  ...
                  {targetCurrency !== group.baseCurrency && (
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <input type="number" inputMode="decimal" value={exchangeRate} onChange={(e) => setExchangeRate(e.target.value)}
                        className="flex-1 h-9 rounded-lg bg-background px-3 text-sm text-foreground outline-none border border-border focus:ring-1 focus:ring-ios-blue" step="0.001" />
                ...
                <div className="flex gap-1 bg-secondary rounded-xl p-1">
                  {([{ key: "transfers" as const, label: "轉帳清單" }, { key: "stats" as const, label: "統計報表" }]).map((tab) => (
                    <button key={tab.key} onClick={() => setSettleTab(tab.key)}
                      className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${settleTab === tab.key ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"}`}>
                      {tab.label}
                    </button>
                  ))}
                </div>
                ...
                <AnimatePresence mode="wait">
                  {settleTab === "transfers" ? (
                    <motion.div key="transfers" className="flex flex-col gap-2"
                      ...
                      {settlements.length === 0 ? (
                        ...
                        <motion.div key={`${s.from}-${s.to}`}
                          ...
                          <button onClick={() => toggleSettled(i)}
                    ...
                    <motion.div key="stats" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.2 }}>
                      <SettlementStats expenses={expenses} members={groupMembers} currencySymbol={isConverting ? targetCurrencySymbol : baseCurrencySymbol} exchangeRate={rate} />
                    </motion.div>
                  )}
                </AnimatePresence>
              </motion.div>
                  ...
                  <motion.div key={exp.id} className="ios-card px-4 py-3 flex items-center gap-3"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {exp.splitAmong.length}人分擔 · 每人{baseCurrencySymbol}{Math.round(exp.amount / exp.splitAmong.length)}
                      </p>
                    </div>
                    <p className="font-semibold text-foreground">{baseCurrencySymbol}{exp.amount}</p>
