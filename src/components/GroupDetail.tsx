...
export function GroupDetail({ group, onBack, onGroupUpdate }: Props) {
  ...
  const [expenses, setExpenses] = useState<GroupExpense[]>(() =>
    getGroupExpensesByGroup(group.id)
  );
  ...
  const toggleGroupMember = (memberId: string) => {
    ...
    const updated = updateGroup(group.id, { memberIds: newIds });
    ...
    <div className="flex flex-col gap-4 pb-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground">{groupMembers.length} 位成員</p>
        </div>
      </div>
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all active:scale-[0.98] ${
                        inGroup ? "bg-ios-blue/10" : "hover:bg-accent"
                      }`}
                    >
                      ...
                      <div
                        className={`w-6 h-6 rounded-full flex items-center justify-center transition-all ${
                          inGroup
                            ? "bg-ios-blue text-primary-foreground"
              ...
              >
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`w-4 h-4 text-muted-foreground transition-transform ${showPayerDropdown ? "rotate-180" : ""}`}
                    />
                  </button>
                  <AnimatePresence>
                    {showPayerDropdown && (
                ...
                <AnimatePresence mode="wait">
                  {settleTab === "transfers" ? (
                          ...
                          >
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-medium text-foreground">
                                {memberEmoji(s.from)} {memberName(s.from)} → {memberEmoji(s.to)}{" "}
                                {memberName(s.to)}
                              </p>
                    ...
                    <motion.div
                      ...
                      transition={{ duration: 0.2 }}
                    >
                      <SettlementStats expenses={expenses} members={groupMembers} />
                    </motion.div>
                  )}
                </AnimatePresence>
              </motion.div>
