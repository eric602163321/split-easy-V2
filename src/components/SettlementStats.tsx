...
export function SettlementStats({ expenses, members }: Props) {
  const { totalSpend, categoryData, memberSpends } = useMemo(
    () => aggregateData(expenses, members),
    [expenses, members]
  );
        ...
        <div className="flex flex-col">
          {memberSpends.map((ms, idx) => {
            ...
            return (
              ...
              >
                <button
                  onClick={() => setExpandedId(isOpen ? null : ms.member.id)}
                ...
                >
                  ...
                  <span className="font-semibold text-foreground text-sm">${ms.total.toLocaleString()}</span>
                  <ChevronDown
                        ...
                        <div key={cat.key} className="flex items-center gap-2 text-sm">
                          ...
                          <div className="w-16 h-1.5 rounded-full bg-secondary overflow-hidden">
                            <motion.div
                              ...
                              style={{ backgroundColor: CHART_COLORS[ci] }}
                              initial={{ width: 0 }}
                              animate={{ width: `${pct}%` }}
                              transition={{ duration: 0.5, delay: 0.1 }}
                            />
                          </div>
                          <span className="font-medium text-foreground w-16 text-right">${val}</span>
                        </div>
                      );
                    })}
                  </div>
